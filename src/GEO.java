...
public class GEO {
    ...
    public JSONObject distanceRoutes(JSONArray routes, String blunder, String google_key) {
        ...
        for (int itemID = 0; itemID < routes.length(); itemID++) {
            ...
            if (itemID > 0) {
                ...
                try {
                    tDistance = distanceGet(lLt, lLn, route.getString("lt"), route.getString("ln"), blunder, google_key);
                    ...
                    if (google_key != null && !google_key.equals("")) { // если задан ключ, то просчиываем все расстояние, которые могут понадобиться
                        ...
                        for (int itemID2 = 0; itemID2 < distancesToCalc.length(); itemID2++) {
                            ...
                            if (distanceToCalc.getString("status").equals("calc")){
                                distanceGet(distanceToCalc.getString("blt"), distanceToCalc.getString("bln"), distanceToCalc.getString("elt"), distanceToCalc.getString("eln"), "0", google_key);
                            }
                        }
                    }
    ...
    public JSONObject distanceGet(String blt, String bln, String elt, String eln, String blunder, String google_key) throws CacheException {
        ...
        if (DBAnswer.getString("status").equals("calc") && google_key != null && !google_key.equals("")) {
            ...
            if (respJSON.getString("status").equals("OK")) {
                ...
                DBAnswer = new JSONObject(GEO2.DistanceSet(dataBase, blt, bln, elt, eln,
                        AppServer.JSONGetString(result,"distance"),
                        AppServer.JSONGetString(result, "duration"),
                        AppServer.JSONGetString(result, "status")));
            }
        }
        return DBAnswer;
    }
